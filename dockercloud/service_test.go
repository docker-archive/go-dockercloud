...
func Test_ListServices(t *testing.T) {
	...
	defer server.Close()
	url := server.URL + "/api/app/" + appSubsystemVersion + "/service/"
...
func Test_GetService(t *testing.T) {
	...
	defer server.Close()
	url := server.URL + "/api/app/" + appSubsystemVersion + "/service/" + fake_uuid_service
...
func Test_CreateService(t *testing.T) {
	...
	BaseUrl = server.URL + "/api/"
	test_response, err := CreateService(ServiceCreateRequest{Image: "tutum/hello-world", Name: "test", Target_num_containers: 2})
	if err != nil {
		t.Fatal(err)
	}
	if reflect.DeepEqual(test_response, response) != true {
